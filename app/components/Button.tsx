...
const ButtonZiva: React.FC<CustomButtonProps> = ({onClick, className, buttonTextClassName, buttonText, type, leftIcon, rightIcon, textOnly = false, variant, style, iconOnly ,them}) => {
  ...
  return (
    <Button
      ...
      className={`flex h-[48px] px-6 justify-center items-center gap-2.5 self-stretch ${ButtonThem} bg-orange-500 hover:bg-orange-600 text-white  font-bold rounded-xl w-full ${textOnly ? "py-2" : "py-2 px-4"
        } ${className}`}
    >
