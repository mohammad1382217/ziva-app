...
const BuyTicket: React.FC<BuyTicketType> = ({
    ...
    return (
        <div>
            <div className={`relative rounded-3xl  p-6 w-full ${className}`} style={{background:background}}>
                ...
                <div className="relative md-max:gap-6 flex flex-row md-max:flex-col justify-between text-white">
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="flex flex-row  md-max:justify-between gap-6">
                            ...
                            <div className="relative text-2xl font-black text-white">
                                {price} تومان
                            </div>
                        </div>
                        <div>
                            {childrenDiscount && childrenPrice &&
                                <div className="flex-grow">
                                    ...
                                    <div className="flex flex-row-reverse justify-start md-max:flex-col-reverse">
                                        ...
                                        <span className="text-lg font-bold ml-3 md-max:ml-1 md-max:self-start">      بلیط خردسال با {childrenDiscount} درصد تخفیف </span>
                                    </div>
                                </div>
                            }
                        </div>
                    ...
                    <div className="my-auto">
                        <ButtonZiva style={{color:background}} buttonText="خرید بلیط" type="button" className={`w-32 sm-max:w-full h-11 text-[${background}] font-semibold text-sm hover:bg-transparent bg-gradient-to-r from-white/90 to-white/80 bg-opacity-20 backdrop-blur-lg`} leftIcon={<Ticket color={background} height={13} width={16} />} />
                    </div>
                </div>
            </div>
        </div>
