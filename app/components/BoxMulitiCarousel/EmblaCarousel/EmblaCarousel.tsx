...
const EmblaCarousel: React.FC<PropType> = (props) => {
  ...
  return (
    <section className={`embla sm-max:scale-[0.9]`}>
      <div
        className={` ${Verticalable ? "!overflow-visible embla__viewport" : "embla__viewport"} `}
        ref={emblaRef}
      >
        <div
          className="mt-10 embla__container"
          style={{ touchAction: "pan-y pinch-zoom" }}
        >
          {slides.map(
            ({ rate, like, discount, name, location, navigate, price }, index) => (
              <div
                className={`${Verticalable && isChangeVerticalable ? "w-[556px]" : "w-[246px]"} mx-2 flex justify-center`}
                key={index}
              >
                <Card
                  Verticalable={Verticalable && isChangeVerticalable}
                  discount={discount}
                  like={like}
                  location={location}
                  name={name}
